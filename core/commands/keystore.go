...
var KeyCmd = &cmds.Command{
	Helptext: cmdkit.HelpText{
		...
		ShortDescription: `
'ont-ipfs key gen' generates a new keypair for usage with IPNS and 'ont-ipfs name
publish'.

  > ont-ipfs key gen --type=rsa --size=2048 mykey
  > ont-ipfs name publish --key=mykey QmSomeHash

'ont-ipfs key list' lists the available keys.
